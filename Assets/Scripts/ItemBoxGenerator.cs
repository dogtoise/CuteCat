...
public class ItemBoxGenerator : MonoBehaviour
{
    ...
    [SerializeField]
    public ItemGenDesc[] itemGenDescs;

    private bool isStarted = false;

    public int CreateOffset = 10;
        ...
        for (int i = 0; i < itemGenDescs.Length; i++)
        {
            ...
            if (itemGenDescs[i].isStarted == false)
            {
                if (itemGenDescs[i].generateStartTime < itemGenDescs[i].elapsedTime)
                {
                    ...
                    Vector3 dir =  player.preDir.normalized * CreateOffset;
                    obj.transform.position = player.transform.position + dir;
